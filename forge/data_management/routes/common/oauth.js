...
class OAuth {
	...
	getClient(scopes = config.scopes.internal) {
		...
		return AuthClientThreeLegged(client_id, client_secret, callback_url, scopes);
	}
	...
	async setCode(code) {
		const internalTokenClient = this.getClient(config.credentials.internal);
		const publicTokenClient = this.getClient(config.credentials.public);
	...
	_expiresIn() {
		const now = new Date();
		const expiresAt = new Date(this._session_expires_at);
	...
	async _refreshToken() {
		...
		const internalCredentials = await
		internalTokenClient.refreshToken({ refresh_token: this._session.refresh_token });
		const publicCredentials = await publicTokenClient.refreshToken(internalCredentials);
