...
router.get('/buckets', async (req, res, next) => {
	...
	if (!bucket_name || '#' === bucket_name) {
		try {
			const buckets = await new BucketsApi()
		          .getBuckets({ limit: 64 }, req.oauth_client, req.oauth_token);
                  //.getBuckets({ limit: 64, region: 'EMEA' }, req.oauth_client, req.oauth_token);
		...
		try {
			const objects = await new ObjectsApi()
				  .getObjects(bucket_name, {}, req.oauth_client, req.oauth_token);
...
router.post('/objects', multer({ dest: 'uploads/' }).single('fileToUpload'), async (req, res, next) => {
  debugger;
	fs.readFile(req.file.path, async (err, data) => {
		if (err) { next(err); }
		try {
			await new ObjectsApi().uploadObject(req.body.bucketKey, req.file.originalname,
												data.length, data, {}, req.oauth_client, req.oauth_token);
			res.status(200).end();
		}
		catch (err) { return next(err); }
	});
